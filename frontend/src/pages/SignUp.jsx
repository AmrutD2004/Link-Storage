...
import { userRegister } from '../api/endpoint'
...
const SignUp = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.email) {
            toast("Please fill email fields!", {
                icon: <TriangleAlert size={18} />,
                style: {
                    border: "1px solid #95958C",
                    padding: "8px",
                ...
                <div className='w-110 px-5 py-6 shadow-xl rounded-lg mx-3 lg:mx-0'>
                    <form onSubmit={handleSubmit} className='flex flex-col items-center justify-between h-full'>
                        ...
                        <div className='flex flex-col items-start justify-start gap-3 w-full px-5 py-3'>
                            ...
                            <div className='flex flex-col items-center justify-start gap-2 w-full relative'>
                                <label className="text-sm font-medium text-start w-full text-[#0B3A66]">Email</label>
                                <Mail size={16} className='absolute top-9 left-3 text-neutral-300' />
                                <input onChange={handleChange} className='w-full border border-neutral-300 rounded-lg px-10 py-1.5 text-sm placeholder:text-neutral-400 outline-none' type="text" name='email' value={formData.email} placeholder='Enter username' />
                            </div>
                        ...
                        <div className='flex items-center justify-center w-full px-5 py-3'>
                            {loading ? <button className='w-full text-white font-medium tracking-tight text-sm flex items-center justify-center gap-1 py-3 rounded-lg cursor-pointer bg-[#0E4A82] '><Loader2 size={18} className='animate-spin' />Creating Account...</button>
                                :
                                <button className='w-full text-white font-medium tracking-tight text-sm bg-[#0B3A66] py-3 rounded-lg shadow cursor-pointer hover:bg-[#0E4A82] transition-colors duration-200'>Create Account</button>
                            }

                        </div>
                        <h1 className='text-sm font-medium'>Already have account? <Link to={'/login'} className='text-[#0B3A66] font-semibold underline'>Sign In</Link></h1>
                    </form>
                </div>
            </div>
        </div>
